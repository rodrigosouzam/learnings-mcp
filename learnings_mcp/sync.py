...
def export_to_dir(d=None, workspaces=None) -> int:
    """Export learnings to the repo. If `workspaces` (a set of names) is given, only
    those are exported — e.g. {'base', 'acme'} to share one project with a team without
    leaking your other workspaces."""
    root = sync_dir(d)
    root.mkdir(parents=True, exist_ok=True)
    store = Store()
    recs = store.all_records()
    if workspaces:
        recs = [r for r in recs if r["workspace"] in workspaces]
    written = set()
    for r in recs:
        ...
        written.add(r["id"])
    # Reflect local deletions — but a scoped export only prunes within its own
    # workspaces, so it never touches other workspaces' files in a shared repo.
    for p in root.glob("*/*.md"):
        if workspaces and p.parent.name not in workspaces:
            continue
...
def import_from_dir(d=None, workspaces=None) -> dict:
    root = sync_dir(d)
    store = Store()
...
def sync(d=None, message="learnings sync", workspaces=None) -> str:
    root = sync_dir(d)
    root.mkdir(parents=True, exist_ok=True)
    ...
    n = export_to_dir(root, workspaces)
    _git(root, "add", "-A", check=False)
    ...
    stats = import_from_dir(root, workspaces)
